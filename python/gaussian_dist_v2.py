...
def plot():
    ...
    #define constants
    mean = np.mean(Co60_20000_events)
    variance = np.var(Co60_20000_events)
    sigma = np.sqrt(variance)
    mean_of_events = np.mean(Co60_events_between_1150_1350)
    
    LSL = min(Co60_20000_events)
    USL = max(Co60_20000_events)
    num_bins = 80
    ...
    n, bins, patches = ax.hist(Co60_20000_events, bins = num_bins, range=(-200,200),density=True,  edgecolor="blue",color='white')
    ...
    plt.annotate("NoM = {0:.4g}".format(len(Co60_20000_events)), xy=(0, 1), xytext=(12, -72), va='top',xycoords='axes fraction', textcoords='offset points')
