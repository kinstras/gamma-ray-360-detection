...
class Co57_isotope:
    ...
    class Co57_AnalyticalMethodSolution_12x12_SiPM:
        ...
        def __init__(self):
            ...
            self.data_folder = Path("/home/konstantinos/Desktop/diplomatiki/main_code/Co_57_Folder/realistic_geometry_detectors/Co57_12x12_SiPM_10000_events/Co57_12x12_SiPM_10000_events_txt/")
    ...
    class Co57_DataAnalysis_12x12_SiPM:
        ...
        def gaussian_plot_distribution(self):
            ...
            n, bins_ax, patches = plt.hist(Co57_3500_events_btin_110_140kev, bins = num_bins, range=(-20,20),density=True,  edgecolor="blue",color='white')

            
            y1 = ((1 / (np.sqrt(2 * np.pi) * sigma1)) *
                 np.exp(-0.5 * (1 / sigma1 * (bins_ax - mean1))**2))
            
            y2 = ((1 / (np.sqrt(2 * np.pi) * sigma2)) *
                 np.exp(-0.5 * (1 / sigma2 * (bins_ax - mean2))**2))
            
            y3 = ((1 / (np.sqrt(2 * np.pi) * sigma3)) *
                 np.exp(-0.5 * (1 / sigma3 * (bins_ax - mean3))**2))
            
            y4 = ((1 / (np.sqrt(2 * np.pi) * sigma4)) *
                 np.exp(-0.5 * (1 / sigma4 * (bins_ax - mean4))**2))
            
            # Define each line separately
            line1, = plt.plot(bins_ax, y1, '--',label='photopeak: 3.500 event', color='black')
            line2, = plt.plot(bins_ax, y2, '--',label='photopeak: 5.000 event', color='red')
            line3, = plt.plot(bins_ax, y3, '--',label='photopeak: 10.000 event', color='blue')
            line4, = plt.plot(bins_ax, y4, '--',label='photopeak: 15.000 event', color='purple')

            #filled area
            ax.fill(bins_ax, y1,facecolor ='black', alpha = 0.2)
            ax.fill(bins_ax, y2,facecolor ='red', alpha = 0.2)
            ax.fill(bins_ax, y3,facecolor ='blue', alpha = 0.2)
            ax.fill(bins_ax, y4,facecolor ='purple', alpha = 0.2)
            ax.set_xlim([-10,10])
...
class Cs137_isotope():
    ...
    class Cs137_DataAnalysis_12x12_SiPM:
        ...
        def gaussian_plot_distribution(self):
            ...
            n, bins_ax, patches = plt.hist(Cs137_3500_events_btin_630_700kev, bins = num_bins, range=(-200,200),density=True,  edgecolor="blue",color='white')
...
class Co60_isotope:
    ...
    class Co60_DataAnalysis_12x12_SiPM:
        ...
        def gaussian_plot_distribution(self):
            ...
            n, bins_ax, patches = plt.hist(Co60_14200_events_btin_1150_1350kev, bins = num_bins, range=(-200,200),density=True,  edgecolor="blue",color='white')
    ...
    class Co60_DataAnalysis_8x8_SiPM:
        ...
        def gaussian_plot_distribution(self):
            ...
            n, bins_ax, patches = plt.hist(Co60_15000_events_btin_1150_1350kev, bins = num_bins, range=(-200,200),density=True,  edgecolor="blue",color='white')
